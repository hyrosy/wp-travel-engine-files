...
if ( isset( $coupons[ 0 ] ) ) : // Checks if has at least one coupon applied.
	foreach ( $coupons as $coupon ) :
		...
		<div class="wpte-checkout__coupon-card">
			<div class="wpte-checkout__coupon-title"><?php esc_html_e( "YAY! You saved ", 'wp-travel-engine' );
				wptravelengine_the_price( $coupon[ 'amount' ] ?? 0 ); ?></div>
			<div class="wpte-checkout__coupon-content"><?php esc_html_e( "Coupon ", 'wp-travel-engine' );
				echo esc_html( $coupon[ 'label' ] ?? '' );
				esc_html_e( " Applied", 'wp-travel-engine' ); ?></div>
