...
class Customer extends PostType {
	...
	public function save( $post_id, $post, $update ) {
		...
		if ( empty( array_intersect( array_map( fn( $action ) => wp_verify_nonce( $nonce, $action ), $valid_actions ), [ true ] ) ) ) {
			return new WP_Error( 'invalid_nonce', __( 'Invalid nonce', 'wp-travel-engine' ) );
		}

		$my_modal = new CustomerModel( $post_id );

		$my_modal->set_customer_details( $params['wp_travel_engine_booking_setting']['place_order']['booking'] ?? $params['billing'] ?? [] );
	...
	public function meta_box_customer_callback() {
		...
		wptravelengine_get_admin_template( 'customer/index.php',
			array(
	...
	private function get_sidebar_tabs() {
		return apply_filters( 'wptravelengine_customer_sidebar_tabs', [
			[
				'class' => 'is-active',
				'target' => 'profile',
	...
	private function get_infos_details( $infos ) {
		return apply_filters( 'wptravelengine_customer_infos_details', [
			[
	...
	private function get_address_details( $addresses ) {
		return apply_filters( 'wptravelengine_customer_address_details', [
			[
